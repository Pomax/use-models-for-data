...
```
$ node ./your/data/store/metaname/Classname.vN.to.vM.js ./your/data/store/metaname/some-object.json
```

This will apply a migration but write the result to `stdout` rather than back to the same file, so you can verify that the result is indeed what it should be.
