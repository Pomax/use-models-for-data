...
export class Fields {
  ...
  static choice = function (choices, options = {}) {
    if (choices === undefined || !(choices instanceof Array)) {
      throw new Error(`Missing choices array for choice field.`);
    }
    return new ModelField({ type: undefined, choices, ...options });
  };

  static model = function (shape) {
    if (shape.__meta === undefined && shape.model) {
      const { model, ...options } = shape;
      return new ModelField({ shape: model, ...options });
    }
    return new ModelField({ shape });
  };
}
...
export function validate(key, value, definition, strict = false) {
  ...
  try {
    if (customValidate(value) === false) throw new Error();
    return { passed: true };
  } catch (err) {
