...
class ModelRegistry {
  ...
  async recordModelClassWithStoreBacking(BaseModel) {
    ...
    const handleModel = async (Model) => {
      ...
      if (schema.__meta.distinct) {
        if (!stored) {
          // console.log(`handleModel (${modelName}): not yet stored`);
        ...
        if (stored) {
          ...
          // console.log(
          //   `handleModel (${modelName}): ${stored.__meta.version}, ${schema.__meta.version}`
          // );
      ...
      // console.log(`schema (${modelName}) version: ${schema.__meta.version}`);

      if (!REGISTER[modelName]) {
        REGISTER[modelName] = Model.schema = schema;
        // console.log(`bound ${schema.__meta.version}`);
      }
    };
    ...
    if (diffList.length > 0) {
      ...
      while (mfList.length) {
        const { stored, schema } = mfList.shift();
        await this.generateMigrationFile(stored, schema);
      }
   ...
   */
  async generateMigrationFile(schema1, schema2) {
